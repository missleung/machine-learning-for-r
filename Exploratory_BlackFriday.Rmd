...
```{r}
# Occupational
ggplot(dat_User, aes(x = Occupation, y=sum_Purchase)) +
  geom_col()

ggplot(dat_User, aes(x = City_Category, y=Occupation)) +
  geom_mosaic()
```

It may seem that the twenty different occupations could potentially be a factor in purchasing amount per person.
```{r}
# How do the categories compare with the amount spent per person?
dat_Raw

ggplot(dat_spendperperson_city, aes(x = Category, y=avg_PurchasePerPerson)) +
  geom_col()
```